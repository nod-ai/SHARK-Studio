...
def get_client(serialize=True):
    ...
    client = Client(
        os.getenv("HOST", "http://localhost:7860"), serialize=serialize
    )
    if debug:
...
def get_args(
    ...
    kwargs = OrderedDict(
        instruction=prompt if chat else "",  # only for chat=True
    ...
    assert (
        len(set(eval_func_param_names).difference(set(list(kwargs.keys()))))
        == 0
    )
    if chat:
...
@pytest.mark.skip(
    reason="For manual use against some server, no server launched"
)
def test_client_basic(prompt_type="human_bot"):
    return run_client_nochat(
        prompt="Who are you?", prompt_type=prompt_type, max_new_tokens=50
    )


def run_client_nochat(prompt, prompt_type, max_new_tokens):
    kwargs, args = get_args(
        prompt, prompt_type, chat=False, max_new_tokens=max_new_tokens
    )
    ...
    res_dict = dict(
        prompt=kwargs["instruction_nochat"],
        iinput=kwargs["iinput_nochat"],
...
def test_client_basic_api_lean(prompt_type="human_bot"):
    return run_client_nochat_api_lean(
        prompt="Who are you?", prompt_type=prompt_type, max_new_tokens=50
    )
...
def test_client_basic_api_lean_morestuff(prompt_type="human_bot"):
    return run_client_nochat_api_lean_morestuff(
        prompt="Who are you?", prompt_type=prompt_type, max_new_tokens=50
    )


def run_client_nochat_api_lean_morestuff(
    prompt, prompt_type="human_bot", max_new_tokens=512
):
    kwargs = dict(
        instruction="",
        iinput="",
...
def run_client(
    client, prompt, args, kwargs, do_md_to_text=True, verbose=False
):
    ...
    if not kwargs["stream_output"]:
        ...
        res_dict["response"] = md_to_text(
            full_outputs[-1][0][0][1], do_md_to_text=do_md_to_text
        )
        return res_dict, client
