...
from apps.language_models.langchain.gradio_utils.css import get_css
from apps.language_models.langchain.gradio_utils.prompt_form import (
    make_prompt_form,
    make_chatbots,
)
...
from apps.language_models.langchain.gradio_themes import (
    H2oTheme,
    SoftTheme,
    get_h2o_title,
    get_simple_title,
...
from apps.language_models.langchain.prompter import (
...
from apps.language_models.langchain.utils import (
    get_githash,
    flatten_list,
    zip_data,
    s3up,
...
from apps.language_models.langchain.gen import (
    get_model,
    languages_covered,
    evaluate,
    score_qa,
    ...
    get_minmax_top_k_docs,
    history_to_context,
    langchain_actions,
)
from apps.language_models.langchain.evaluate_params import (
...
def go_gradio(**kwargs):
    ...
    if "mbart-" in kwargs["model_lower"]:
        ...
        instruction_label_nochat = (
            "Instruction (Shift-Enter or push Submit to send message,"
    ...
    if kwargs["gradio_offline_level"] >= 0:
        ...
        if kwargs["gradio_offline_level"] == 1:
            ...
            base_font = "Helvetica"
        theme_kwargs = dict(
            font=(base_font, "ui-sans-serif", "system-ui", "sans-serif"),
            font_mono=(
                "IBM Plex Mono",
                "ui-monospace",
                "Consolas",
    ...
    if kwargs["gradio_size"] == "xsmall":
        theme_kwargs.update(
            dict(
                spacing_size=spacing_xsm,
                text_size=text_xsm,
                radius_size=radius_xsm,
            )
        )
    ...
    theme = (
        H2oTheme(**theme_kwargs)
        if kwargs["h2ocolors"]
        else SoftTheme(**theme_kwargs)
    )
    demo = gr.Blocks(
        theme=theme, css=css_code, title="h2oGPT", analytics_enabled=False
    )
    callback = gr.CSVLogger()

    model_options = (
        flatten_list(list(prompt_type_to_model_name.values()))
        + kwargs["extra_model_options"]
    )
    ...
    output_label0 = (
        f'h2oGPT [Model: {kwargs.get("base_model")}]'
    ...
    (
        default_kwargs["prompt_type"],
        default_kwargs["prompt_dict"],
    ) = update_prompt(
        default_kwargs["prompt_type"],
        default_kwargs["prompt_dict"],
    ...
    with demo:
        ...
        docs_state00 = kwargs["document_choice"] + [
            x.name for x in list(DocumentChoices)
        ]
        docs_state0 = []
        ...
        base_wanted = (
            kwargs["base_model"] != no_model_str
            and kwargs["login_mode_if_model0"]
        )
        go_btn = gr.Button(
        ...
        res_value = (
            "Response Score: NA"
            if not kwargs["model_lock"]
            else "Response Scores: %s" % nas
        )
        ...
        with normal_block:
            with gr.Tabs():
                with gr.Row():
                    ...
                    with (
                        col_nochat
                    ):  # FIXME: for model comparison, and check rest
                        if kwargs["langchain_mode"] == "Disabled":
                            text_output_nochat = gr.Textbox(
                                lines=5,
                                label=output_label0,
                        ...
                        iinput_nochat = gr.Textbox(
                            lines=4,
                            label="Input context for Instruction",
                            placeholder=kwargs["placeholder_input"],
                        )
                        submit_nochat = gr.Button("Submit")
                        flag_btn_nochat = gr.Button("Flag")
                        with gr.Column(visible=kwargs["score_model"]):
                            score_text_nochat = gr.Textbox(
                                "Response Score: NA", show_label=False
                            )

                    col_chat = gr.Column(visible=kwargs["chat"])
                    with col_chat:
                        instruction, submit, stop_btn = make_prompt_form(
                            kwargs, LangChainMode
                        )
                        (
                            text_output,
                            text_output2,
                            ...
                            output_label0, output_label0_model2, **kwargs
                        )
                        ...
                        with gr.Row():
                            ...
                            with gr.Column(visible=kwargs["score_model"]):
                                score_text = gr.Textbox(
                                    res_value, show_label=False, visible=True
                                )
                                score_text2 = gr.Textbox(
                                    "Response Score2: NA",
                                    show_label=False,
                    ...
                    inputs_dict_str = gr.Textbox(
                        label="API input for nochat",
                        show_label=False,
                        visible=False,
                    )
                    text_output_nochat_api = gr.Textbox(
                        lines=5,
                        label="API nochat output",
                        visible=False,
                        show_copy_button=True,
                    )
                with gr.TabItem("Documents"):
                    langchain_readme = get_url(
                        "https://github.com/h2oai/h2ogpt/blob/main/docs/README_LangChain.md",
                        from_str=True,
                    )
                    gr.HTML(
                        value=f"""LangChain Support Disabled<p>
                            Run:<p>
                            <code>
                    ...
                    with data_row1:
                        ...
                        if (
                            ...
                            and "MyData" not in allowed_modes
                        ):
                            allowed_modes += ["MyData"]
                        if (
                            allow_upload_to_user_data
                            and "UserData" not in allowed_modes
                        ):
                            allowed_modes += ["UserData"]
                        langchain_mode = gr.Radio(
                            [
                                x
                                for x in langchain_modes
                                if x in allowed_modes
                                and x not in no_show_modes
                            ],
                        ...
                        allowed_actions = [
                            x
                            for x in langchain_actions
                            if x in visible_langchain_actions
                        ]
                        langchain_action = gr.Radio(
                            allowed_actions,
                            value=allowed_actions[0]
                    ...
                    with data_row2:
                        ...
                        with gr.Row(
                            visible=kwargs["langchain_mode"] != "Disabled"
                            and enable_sources_list
                        ):
                            get_sources_btn = gr.Button(
                                value="Get Sources", scale=0, size="sm"
                            )
                            show_sources_btn = gr.Button(
                                value="Show Sources", scale=0, size="sm"
                            )
                            refresh_sources_btn = gr.Button(
                                value="Refresh Sources", scale=0, size="sm"
                            )
                    ...
                    with upload_row:
                        with gr.Column():
                            ...
                            with gr.Row():
                                add_to_shared_db_btn = gr.Button(
                                    "Add File(s) to UserData",
                                    visible=allow_upload_to_user_data,
                                    elem_id="small_btn",
                                )
                                add_to_my_db_btn = gr.Button(
                                    "Add File(s) to Scratch MyData",
                                    ...
                                    size="sm"
                                    if not allow_upload_to_user_data
                                    else None,
                                )
                        with gr.Column(
                            visible=kwargs["langchain_mode"] != "Disabled"
                            and allow_upload
                            and enable_url_upload
                        ):
                            url_label = (
                                "URL (http/https) or ArXiv:"
                                if have_arxiv
                                else "URL (http/https)"
                            )
                            url_text = gr.Textbox(
                                label=url_label,
                                placeholder="Click Add to Submit"
                            ...
                            with gr.Row():
                                url_user_btn = gr.Button(
                                    value="Add URL content to Shared UserData",
                                    visible=allow_upload_to_user_data
                                    and allow_upload_to_my_data,
                                    elem_id="small_btn",
                                )
                                url_my_btn = gr.Button(
                                    value="Add URL content to Scratch MyData",
                                    visible=allow_upload_to_my_data
                                    and allow_upload_to_user_data,
                                    elem_id="small_btn"
                                    if allow_upload_to_user_data
                        ...
                        with gr.Column(
                            visible=kwargs["langchain_mode"] != "Disabled"
                            and allow_upload
                            and enable_text_upload
                        ):
                            user_text_text = gr.Textbox(
                                label="Paste Text [Shift-Enter more lines]",
                                placeholder="Click Add to Submit"
                                if allow_upload_to_my_data
                                and allow_upload_to_user_data
                                else "Enter to Submit, Shift-Enter for more lines",
                                interactive=True,
                            )
                            with gr.Row():
                                user_text_user_btn = gr.Button(
                                    value="Add Text to Shared UserData",
                                    visible=allow_upload_to_user_data
                                    and allow_upload_to_my_data,
                                    elem_id="small_btn",
                                )
                                user_text_my_btn = gr.Button(
                                    value="Add Text to Scratch MyData",
                                    visible=allow_upload_to_my_data
                                    and allow_upload_to_user_data,
                                    elem_id="small_btn"
                                    if allow_upload_to_user_data
                        ...
                        with gr.Column(visible=False):
                            ...
                            with gr.Row(visible=False, equal_height=False):
                                ...
                                with gr.Row(visible=True):
                                    github_shared_btn = gr.Button(
                                        value="Add Github to Shared UserData",
                                        visible=allow_upload_to_user_data,
                                        elem_id="small_btn",
                                    )
                                    github_my_btn = gr.Button(
                                        value="Add Github to Scratch MyData",
                                        visible=allow_upload_to_my_data,
                                        elem_id="small_btn",
                                    )
                    sources_row = gr.Row(
                ...
                with gr.TabItem("Chat History"):
                    ...
                    with gr.Row():
                        clear_chat_btn = gr.Button(
                            value="Clear Chat", visible=True, size="sm"
                        )
                        export_chats_btn = gr.Button(
                            value="Export Chats to Download", size="sm"
                        )
                        remove_chat_btn = gr.Button(
                            value="Remove Selected Chat",
                            visible=True,
                            size="sm",
                        )
                        add_to_chats_btn = gr.Button(
                            "Import Chats from Upload", size="sm"
                        )
                    with gr.Row():
                        chats_file = gr.File(
                            interactive=False, label="Download Exported Chats"
                        )
                        chatsup_output = gr.File(
                            label="Upload Chat File(s)",
                            file_types=[".json"],
                            file_count="multiple",
                            elem_id="warning",
                            elem_classes="feedback",
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            prompt_type = gr.Dropdown(
                                prompt_types_strings,
                                value=kwargs["prompt_type"],
                                label="Prompt Type",
                                visible=not kwargs["model_lock"],
                                interactive=not is_public,
                            )
                            prompt_type2 = gr.Dropdown(
                                prompt_types_strings,
                                value=kwargs["prompt_type"],
                            ...
                            if os.getenv("TESTINGFAIL"):
                                max_beams = (
                                    8
                                    if not (
                                        memory_restriction_level or is_public
                                    )
                                    else 1
                                )
                            else:
                            ...
                            num_beams = gr.Slider(
                                ...
                                maximum=max_beams,
                                step=1,
                                value=min(max_beams, kwargs["num_beams"]),
                                label="Beams",
                            ...
                            min_new_tokens2 = gr.Slider(
                                ...
                                label="Min output length 2",
                                visible=False and not kwargs["model_lock"],
                            )
                            early_stopping = gr.Checkbox(
                                label="EarlyStopping",
                                info="Stop early in beam search",
                                value=kwargs["early_stopping"],
                            )
                            max_time = gr.Slider(
                                minimum=0,
                                maximum=kwargs["max_max_time"],
                                step=1,
                                value=min(
                                    kwargs["max_max_time"], kwargs["max_time"]
                                ),
                                label="Max. time",
                            ...
                            context = gr.Textbox(
                                lines=3,
                                label="System Pre-Context",
                                info="Directly pre-appended without prompt processing",
                                interactive=not is_public,
                            )
                            chat = gr.components.Checkbox(
                                label="Chat mode",
                                value=kwargs["chat"],
                                visible=not kwargs["model_lock"],
                                interactive=not is_public,
                            )
                            count_chat_tokens_btn = gr.Button(
                                value="Count Chat Tokens",
                                visible=not is_public
                                and not kwargs["model_lock"],
                                interactive=not is_public,
                            )
                            chat_token_count = gr.Textbox(
                                label="Chat Token Count",
                                value=None,
                                visible=not is_public
                                and not kwargs["model_lock"],
                                interactive=False,
                            )
                            chunk = gr.components.Checkbox(
                                value=kwargs["chunk"],
                            ...
                            (
                                ...
                                max_top_k_docs,
                                label_top_k_docs,
                            ) = get_minmax_top_k_docs(is_public)
                            top_k_docs = gr.Slider(
                                minimum=min_top_k_docs,
                                maximum=max_top_k_docs,
                                step=1,
                ...
                with gr.TabItem("Models"):
                    model_lock_msg = gr.Textbox(
                        lines=1,
                        label="Model Lock Notice",
                        placeholder="Started in model_lock mode, no model changes allowed.",
                        visible=bool(kwargs["model_lock"]),
                        interactive=False,
                    )
                    load_msg = (
                        "Load-Unload Model/LORA [unload works if did not use --base_model]"
                        if not is_public
                        else "LOAD-UNLOAD DISABLED FOR HOSTED DEMO"
                    )
                    load_msg2 = (
                        "Load-Unload Model/LORA 2 [unload works if did not use --base_model]"
                        if not is_public
                        else "LOAD-UNLOAD DISABLED FOR HOSTED DEMO 2"
                    )
                    variant_load_msg = (
                        "primary" if not is_public else "secondary"
                    )
                    compare_checkbox = gr.components.Checkbox(
                        label="Compare Mode",
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            with gr.Row():
                                with gr.Column(
                                    scale=20, visible=not kwargs["model_lock"]
                                ):
                                ...
                                with gr.Column(
                                    scale=1, visible=not kwargs["model_lock"]
                                ):
                                    ...
                                    model_gpu = gr.Dropdown(
                                        n_gpus_list,
                                        label="GPU ID [-1 = all GPUs, if Choose is enabled]",
                                        value=kwargs["gpu_id"],
                                        interactive=not is_public,
                                    )
                                    model_used = gr.Textbox(
                                        label="Current Model",
                                        value=kwargs["base_model"],
                                        interactive=False,
                                    )
                                    lora_used = gr.Textbox(
                                        label="Current LORA",
                                        value=kwargs["lora_weights"],
                                        visible=kwargs["show_lora"],
                                        interactive=False,
                                    )
                                    server_used = gr.Textbox(
                                    ...
                                    prompt_dict = gr.Textbox(
                                        label="Prompt (or Custom)",
                        ...
                        with col_model2:
                            with gr.Row():
                                with gr.Column(
                                    ...
                                    lora_choice2 = gr.Dropdown(
                                        lora_options_state.value[0],
                                        label="Choose LORA 2",
                                        value=no_lora_str,
                                        visible=kwargs["show_lora"],
                                    )
                                    server_choice2 = gr.Dropdown(
                                        server_options_state.value[0],
                                ...
                                with gr.Column(
                                    ...
                                    model_used2 = gr.Textbox(
                                        label="Current Model 2",
                                        value=no_model_str,
                                        interactive=False,
                                    )
                                    lora_used2 = gr.Textbox(
                                        label="Current LORA 2",
                                        value=no_lora_str,
                                        visible=kwargs["show_lora"],
                                        interactive=False,
                                    )
                                    server_used2 = gr.Textbox(
                                        label="Current Server 2",
                                        value=no_server_str,
                                        interactive=False,
                                        visible=not is_public,
                                    )
                                    prompt_dict2 = gr.Textbox(
                                        label="Prompt (or Custom) 2",
                                        value=pprint.pformat(
                                            kwargs["prompt_dict"], indent=4
                                        ),
                                        interactive=not is_public,
                    ...
                    with gr.Row(visible=not kwargs["model_lock"]):
                        with gr.Column(scale=50):
                            new_model = gr.Textbox(
                                label="New Model name/path",
                                interactive=not is_public,
                            )
                        with gr.Column(scale=50):
                            new_lora = gr.Textbox(
                                label="New LORA name/path",
                                visible=kwargs["show_lora"],
                                interactive=not is_public,
                            )
                        with gr.Column(scale=50):
                            new_server = gr.Textbox(
                                label="New Server url:port",
                                interactive=not is_public,
                            )
                        with gr.Row():
                            add_model_lora_server_button = gr.Button(
                                "Add new Model, Lora, Server url:port",
                                scale=0,
                                size="sm",
                                interactive=not is_public,
                            )
                with gr.TabItem("System"):
                    ...
                    with admin_row:
                        ...
                        admin_btn = gr.Button(
                            value="Admin Access", visible=is_public
                        )
                        ...
                        with gr.Column():
                            ...
                            with gr.Row():
                                system_input = gr.Textbox(
                                    label="System Info Dict Password",
                                    interactive=True,
                                    visible=not is_public,
                                )
                                system_btn2 = gr.Button(
                                    value="Get System Info Dict",
                                    visible=not is_public,
                                )
                                system_text2 = gr.Textbox(
                                    label="System Info Dict",
                                    interactive=False,
                                    visible=not is_public,
                                    show_copy_button=True,
                                )
                            with gr.Row():
                                system_btn3 = gr.Button(
                                    value="Get Hash", visible=not is_public
                                )
                                system_text3 = gr.Textbox(
                                    label="Hash",
                                    interactive=False,
                                    visible=not is_public,
                                    show_copy_button=True,
                ...
                with gr.TabItem("Disclaimers"):
                    ...
                    gr.Markdown(
                        value=description, show_label=False, interactive=False
                    )
        ...
        # Add to UserData
        update_user_db_func = functools.partial(
            update_user_db,
            dbs=dbs,
            db_type=db_type,
        ...
        add_file_outputs = [
            fileup_output,
            langchain_mode,
            add_to_shared_db_btn,
            add_to_my_db_btn,
        ]
        add_file_kwargs = dict(
            fn=update_user_db_func,
            inputs=[
                fileup_output,
                my_db_state,
                add_to_shared_db_btn,
        ...
        eventdb2b = eventdb2a.then(
            make_non_interactive,
            inputs=add_url_outputs,
        ...
        update_user_db_txt_func = functools.partial(
            update_user_db_func, is_txt=True
        )
        add_text_outputs = [
            user_text_text,
            langchain_mode,
        ...
        eventdb3a = func3(
            fn=dummy_fun,
            inputs=user_text_text,
            outputs=user_text_text,
            queue=queue,
            show_progress="minimal",
        )
        eventdb3b = eventdb3a.then(
            make_non_interactive,
            inputs=add_text_outputs,
        ...
        update_my_db_func = functools.partial(
            update_user_db,
            dbs=dbs,
            db_type=db_type,
        ...
        add_my_file_kwargs = dict(
            fn=update_my_db_func,
            inputs=[
                fileup_output,
                my_db_state,
                add_to_shared_db_btn,
            ...
            queue=queue,
            api_name="add_to_my"
            if allow_api and allow_upload_to_my_data
            else None,
        )
        ...
        eventdb4a = func4(
            make_non_interactive,
            inputs=add_my_file_outputs,
            outputs=add_my_file_outputs,
            show_progress="minimal",
        )
        eventdb4 = eventdb4a.then(
            **add_my_file_kwargs, show_progress="minimal"
        )
        eventdb4.then(
            make_interactive,
            inputs=add_my_file_outputs,
        ...
        update_my_db_url_func = functools.partial(
            update_my_db_func, is_url=True
        )
        add_my_url_outputs = [
            url_text,
            langchain_mode,
        ...
        add_my_url_kwargs = dict(
            fn=update_my_db_url_func,
            inputs=[
                url_text,
                my_db_state,
                url_user_btn,
        ...
        eventdb5b = eventdb5a.then(
            make_non_interactive,
            inputs=add_my_url_outputs,
            outputs=add_my_url_outputs,
            show_progress="minimal",
        )
        ...
        update_my_db_txt_func = functools.partial(
            update_my_db_func, is_txt=True
        )
        ...
        add_my_text_kwargs = dict(
            fn=update_my_db_txt_func,
            inputs=[
                user_text_text,
                my_db_state,
                user_text_user_btn,
        ...
        eventdb6b = eventdb6a.then(
            make_non_interactive,
            inputs=add_my_text_outputs,
            outputs=add_my_text_outputs,
            show_progress="minimal",
        )
        eventdb6 = eventdb6b.then(
            **add_my_text_kwargs, show_progress="minimal"
        )
        eventdb6.then(
            make_interactive,
            inputs=add_my_text_outputs,
        ...
        def clear_doc_choice():
            return gr.Dropdown.update(
                choices=docs_state0, value=[docs_state0[0]]
            )

        langchain_mode.change(
            clear_doc_choice, inputs=None, outputs=document_choice
        )

        def update_dropdown(x):
            return gr.Dropdown.update(choices=x, value=[docs_state0[0]])

        eventdb7 = get_sources_btn.click(
            get_sources1,
            inputs=[my_db_state, langchain_mode],
        ...
        eventdb8 = show_sources_btn.click(
            fn=show_sources1,
            inputs=[my_db_state, langchain_mode],
        ...
        refresh_sources1 = functools.partial(
        ...
        eventdb9 = refresh_sources_btn.click(
            fn=refresh_sources1,
            inputs=[my_db_state, langchain_mode],
            outputs=sources_text,
        ...
        kwargs_evaluate = {
            k: v for k, v in all_kwargs.items() if k in inputs_kwargs_list
        }
        # ensure present
        ...
        def evaluate_nochat(
            *args1, default_kwargs1=None, str_api=False, **kwargs1
        ):
            ...
            if str_api:
                ...
                user_kwargs = {
                    k: v for k, v in zip(eval_func_param_names, args_list[2:])
                }
            ...
            args_list = [
                user_kwargs[k]
                if k in user_kwargs and user_kwargs[k] is not None
                else default_kwargs1[k]
        ...
        exception_text = gr.Textbox(
            value="",
            visible=kwargs["chat"],
            label="Chat Exceptions",
            interactive=False,
        )
        dark_mode_btn.click(
            None,
        ...
        chat.select(
            ...
            chat,
            col_nochat,
            api_name="chat_checkbox" if allow_api else None,
        ).then(col_chat_fun, chat, col_chat).then(
            context_fun, chat, context
        ).then(
            col_chat_fun, chat, exception_text
        )
        ...
        def score_last_response(*args, nochat=False, num_model_lock=0):
            try:
                if num_model_lock > 0:
                    ...
                    return _score_last_response(
                        *args, nochat=nochat, num_model_lock=num_model_lock
                    )
            finally:
                clear_torch_cache()

        def _score_last_response(
            *args, nochat=False, num_model_lock=0, prefix="Response Score: "
        ):
            """Similar to user()"""
            args_list = list(args)
            ...
            )  # restrict deberta related to max for LLM
            ...
            if not nochat:
                ...
                if (
                    smodel is not None
                    and stokenizer is not None
                    and sdevice is not None
        ...
        score_args_nochat = dict(
            fn=partial(score_fun, nochat=True),
            inputs=inputs_list + [text_output_nochat],
            outputs=[score_text_nochat],
        )

        def update_history(
            *args, undo=False, retry=False, sanitize_user_prompt=False
        ):
            """
            ...
            input1 = args_list[
                eval_func_param_names.index("iinput")
            ]  # chat only
            prompt_type1 = args_list[
                eval_func_param_names.index("prompt_type")
            ]
            langchain_mode1 = args_list[
                eval_func_param_names.index("langchain_mode")
            ]
            langchain_action1 = args_list[
                eval_func_param_names.index("langchain_action")
            ]
            document_choice1 = args_list[
                eval_func_param_names.index("document_choice")
            ]
            if not prompt_type1:
            ...
            if user_message1 in ["", None, "\n"]:
                if (
                    langchain_action1 in LangChainAction.QUERY.value
                    and DocumentChoices.Only_All_Sources.name
                    not in document_choice1
                    or langchain_mode1
                    in [LangChainMode.CHAT_LLM.value, LangChainMode.LLM.value]
                ):
        ...
        def all_user(
            *args,
            ...
            for hi, history in enumerate(history_list):
                ...
                hargs += [history]
                history_list[hi] = update_history(
                    *hargs,
                    undo=undo,
                    retry=retry,
                    sanitize_user_prompt=sanitize_user_prompt,
        ...
        def get_model_max_length(model_state1):
            ...
            elif model_state0 and not isinstance(
                model_state0["tokenizer"], str
            ):
        ...
        def prep_bot(*args, retry=False, which_model=0):
            ...
            prompt_dict1 = args_list[
                eval_func_param_names.index("prompt_dict")
            ]

            if (
                model_state1["model"] is None
                or model_state1["model"] == no_model_str
            ):
            ...
            prompt_type1, prompt_dict1 = update_prompt(
                prompt_type1,
                prompt_dict1,
                model_state1,
            ...
            args_list[
                0
            ] = instruction1  # override original instruction with history from user
        ...
        def clear_embeddings(langchain_mode1, my_db):
            ...
            if db_type == "chroma" and langchain_mode1 not in [
                ...
                if db is not None and not isinstance(db, str):
                    clear_embedding(db)
                if (
                    langchain_mode1 == LangChainMode.MY_DATA.value
                    and my_db is not None
                ):
        ...
        def all_bot(*args, retry=False, model_states1=None):
            ...
            args_list0 = args_list[
                : -len(model_states1)
            ]  # same for all models
            exceptions = []
            stream_output1 = args_list[
            ...
            try:
                gen_list = []
                for chatboti, (chatbot1, model_state1) in enumerate(
                    zip(chatbots, model_states1)
                ):
                    ...
                    if (
                        len(chatbot1) > 0
                        and len(chatbot1[-1]) == 2
                        and chatbot1[-1][1] is None
                    ):
                        chatbot1[-1][1] = ""
                    ...
                    history, fun1, langchain_mode1, my_db_state1 = prep_bot(
                        *tuple(args_list1), retry=retry, which_model=chatboti
                    )
                ...
                for res1 in itertools.zip_longest(*gen_list):
                    ...
                    bots = [
                        x[0]
                        if x is not None and not isinstance(x, BaseException)
                        else y
                    ...
                    exceptions_str = "\n".join(
                        [
                            "Model %s: %s" % (iix, choose_exc(x))
                            for iix, x in enumerate(exceptions)
                ...
                if exceptions:
                    ...
                    if exceptions:
                        print(
                            "Generate exceptions: %s" % exceptions, flush=True
                        )
            finally:
                clear_torch_cache()
                clear_embeddings(langchain_mode1, my_db_state1)
        ...
        user_args = dict(
            fn=functools.partial(
                user, sanitize_user_prompt=kwargs["sanitize_user_prompt"]
            ),
            inputs=inputs_list + [text_output],
            outputs=text_output,
        )
        ...
        retry_bot_args = dict(
            fn=functools.partial(bot, retry=True),
            inputs=inputs_list + [model_state, my_db_state] + [text_output],
            outputs=[text_output, exception_text],
        )
        retry_user_args = dict(
            fn=functools.partial(user, retry=True),
            inputs=inputs_list + [text_output],
            outputs=text_output,
        )
        undo_user_args = dict(
            fn=functools.partial(user, undo=True),
            inputs=inputs_list + [text_output],
            outputs=text_output,
        )
        ...
        all_bot_args = dict(
            fn=functools.partial(all_bot, model_states1=model_states),
            inputs=inputs_list + [my_db_state] + text_outputs,
            outputs=text_outputs + [exception_text],
        )
        all_retry_bot_args = dict(
            fn=functools.partial(
                all_bot, model_states1=model_states, retry=True
            ),
            inputs=inputs_list + [my_db_state] + text_outputs,
            outputs=text_outputs + [exception_text],
        )
        all_retry_user_args = dict(
            fn=functools.partial(
                all_user,
                retry=True,
                sanitize_user_prompt=kwargs["sanitize_user_prompt"],
        ...
        if kwargs["model_states"]:
            ...
            for userargs1, botarg1, funn1, funs1 in zip(
                user_args, bot_args, fun_name, fun_source
            ):
                submit_event11 = funs1(
                    fn=dummy_fun,
                    inputs=instruction,
                    outputs=instruction,
                    queue=queue,
                )
                submit_event1a = submit_event11.then(
                    **userargs1,
                    queue=queue,
                    api_name="%s" % funn1 if allow_api else None,
                )
                # if hit enter on new instruction for submitting new query, no longer the saved chat
                submit_event1b = submit_event1a.then(
                    clear_all,
                    inputs=None,
                    outputs=[
                        instruction,
                ...
                submit_event1c = submit_event1b.then(
                    **botarg1,
                    api_name="%s_bot" % funn1 if allow_api else None,
                    queue=queue,
                )
                submit_event1d = submit_event1c.then(
                    **all_score_args,
            ...
            submit_event4 = (
                ...
                .then(
                    **all_undo_user_args,
                    api_name="undo" if allow_api else None,
                )
                .then(
                    clear_all,
                    inputs=None,
                ...
                .then(
                    **all_score_args,
                    api_name="undo_score" if allow_api else None,
                )
            )
            ...
            submit_event11 = instruction.submit(
                fn=dummy_fun,
                inputs=instruction,
                outputs=instruction,
                queue=queue,
            ...
            submit_event1c = submit_event1b.then(
                clear_instruct, None, instruction
            ).then(clear_instruct, None, iinput)
            submit_event1d = submit_event1c.then(
                **bot_args,
                api_name="instruction_bot" if allow_api else None,
                queue=queue,
            )
            submit_event1e = submit_event1d.then(
                **score_args,
                api_name="instruction_bot_score" if allow_api else None,
                queue=queue,
            )
            submit_event1f = submit_event1e.then(
                **bot_args2,
                api_name="instruction_bot2" if allow_api else None,
                queue=queue,
            )
            submit_event1g = submit_event1f.then(
                **score_args2,
                api_name="instruction_bot_score2" if allow_api else None,
                queue=queue,
            )
            ...
            submit_event2c = submit_event2b.then(
                clear_all,
                inputs=None,
                outputs=[
                    instruction,
            ...
            submit_event2d = submit_event2c.then(
                **bot_args,
                api_name="submit_bot" if allow_api else None,
                queue=queue,
            )
            submit_event2e = submit_event2d.then(
                **score_args,
                api_name="submit_bot_score" if allow_api else None,
                queue=queue,
            )
            submit_event2f = submit_event2e.then(
                **bot_args2,
                api_name="submit_bot2" if allow_api else None,
                queue=queue,
            )
            submit_event2g = submit_event2f.then(
                **score_args2,
            ...
            submit_event31 = retry_btn.click(
                fn=dummy_fun,
                inputs=instruction,
                outputs=instruction,
                queue=queue,
            ...
            submit_event3c = submit_event3b.then(
                clear_instruct, None, instruction
            ).then(clear_instruct, None, iinput)
            submit_event3d = submit_event3c.then(
                **retry_bot_args,
                api_name="retry_bot" if allow_api else None,
                queue=queue,
            )
            submit_event3e = submit_event3d.then(
                **score_args,
                api_name="retry_bot_score" if allow_api else None,
                queue=queue,
            )
            submit_event3f = submit_event3e.then(
                **retry_bot_args2,
                api_name="retry_bot2" if allow_api else None,
                queue=queue,
            )
            submit_event3g = submit_event3f.then(
                **score_args2,
            ...
            submit_event4 = (
                ...
                .then(
                    **undo_user_args2, api_name="undo2" if allow_api else None
                )
                .then(
                    clear_all,
                    inputs=None,
                ...
                .then(
                    **score_args, api_name="undo_score" if allow_api else None
                )
                .then(
                    **score_args2,
                    api_name="undo_score2" if allow_api else None,
                )
            )
            submits4 = [submit_event4]
            ...
            for stepx, stepy in zip(x, y):
                ...
                for stepxx, stepyy in zip(stepx, stepy):
                    ...
                    if len(stepyy) != 2:
                        ...
                        return False
                    questionx = (
                        stepxx[0].replace("<p>", "").replace("</p>", "")
                        if stepxx[0] is not None
                        else None
                    )
                    answerx = (
                        stepxx[1].replace("<p>", "").replace("</p>", "")
                        if stepxx[1] is not None
                        else None
                    )

                    questiony = (
                        stepyy[0].replace("<p>", "").replace("</p>", "")
                        if stepyy[0] is not None
                        else None
                    )
                    answery = (
                        stepyy[1].replace("<p>", "").replace("</p>", "")
                        if stepyy[1] is not None
                        else None
                    )
        ...
        def save_chat(*args):
            ...
            chat_list_not_none = [
                x
                for x in chat_list
                if x and len(x) > 0 and len(x[0]) == 2 and x[0][1] is not None
            ]
        ...
        def switch_chat(chat_key, chat_state1, num_model_lock=0):
            ...
            for chati in range(0, 2 + num_model_lock):
                ret_chat[chati % len(ret_chat)] = chosen_chat[
                    chati % len(chosen_chat)
                ]
        ...
        def clear_scores():
            return (
                gr.Textbox.update(value=res_value),
        ...
        radio_chats.input(
            ...
            clear_scores, outputs=[score_text, score_text2, score_text_nochat]
        )
        ...
        remove_chat_btn.click(
            remove_chat, inputs=[radio_chats, chat_state], outputs=chat_state
        ...
        # note for update_user_db_func output is ignored for db
        add_to_chats_btn.click(
            add_chats_from_file,
            inputs=[chatsup_output, chat_state, add_to_chats_btn],
            outputs=[chat_state, add_to_my_db_btn],
            queue=False,
            api_name="add_to_chats" if allow_api else None,
        ).then(clear_file_list, outputs=chatsup_output, queue=False).then(
            update_radio_chats,
            inputs=chat_state,
            outputs=radio_chats,
            queue=False,
        ...
        clear_chat_btn.click(
            ...
            outputs=[text_output, text_output2] + text_outputs,
            queue=False,
            api_name="clear" if allow_api else None,
        ).then(
            deselect_radio_chats, inputs=None, outputs=radio_chats, queue=False
        ).then(
            clear_scores, outputs=[score_text, score_text2, score_text_nochat]
        )
        ...
        submit_event_nochat = (
            submit_nochat.click(
                **no_chat_args, api_name="submit_nochat" if allow_api else None
            )
            .then(clear_torch_cache)
            .then(
                **score_args_nochat,
                api_name="instruction_bot_score_nochat" if allow_api else None,
                queue=queue,
            )
        ...
        submit_event_nochat2 = (
            instruction_nochat.submit(**no_chat_args)
            .then(clear_torch_cache)
        ...
        submit_event_nochat_api = submit_nochat_api.click(
            fun_with_dict_str,
        ...
        def load_model(
            ...
            if kwargs["debug"]:
                print(
                    "Pre-switch pre-del GPU memory: %s"
                    % get_torch_allocated(),
                    flush=True,
                )
            ...
            if (
                isinstance(model_state_old["model"], str)
                and model0 is not None
            ):
                # best can do, move model loaded at first to CPU
                model0.cpu()

            if model_state_old["model"] is not None and not isinstance(
                model_state_old["model"], str
            ):
                try:
                    ...
                    print(
                        "Unable to put model on CPU: %s" % str(e), flush=True
                    )
                del model_state_old["model"]
                model_state_old["model"] = None

            if model_state_old["tokenizer"] is not None and not isinstance(
                model_state_old["tokenizer"], str
            ):
            ...
            if kwargs["debug"]:
                print(
                    "Pre-switch post-del GPU memory: %s"
                    % get_torch_allocated(),
                    flush=True,
                )
            ...
            if model_name is None or model_name == no_model_str:
                ...
                return (
                    [None, None, None, model_name, server_name],
                    model_name,
                    lora_weights,
                    server_name,
                    prompt_type_old,
            ...
            model1, tokenizer1, device1 = get_model(
                reward_type=False,
                **get_kwargs(
                    get_model, exclude_names=["reward_type"], **all_kwargs1
                ),
            )
            clear_torch_cache()
            ...
            if kwargs["debug"]:
                print(
                    "Post-switch GPU memory: %s" % get_torch_allocated(),
                    flush=True,
                )
            return (
                model_state_new,
        ...
        def get_prompt_str(prompt_type1, prompt_dict1, which=0):
            if prompt_type1 in ["", None]:
                print(
                    "Got prompt_type %s: %s" % (which, prompt_type1),
                    flush=True,
                )
                return str({})
            prompt_dict1, prompt_dict_error = get_prompt(
                prompt_type1,
                prompt_dict1,
                chat=False,
                context="",
        ...
        prompt_type.change(
            fn=get_prompt_str_func1,
            inputs=[prompt_type, prompt_dict],
            outputs=prompt_dict,
        )
        prompt_type2.change(
            fn=get_prompt_str_func2,
            inputs=[prompt_type2, prompt_dict2],
        ...
        prompt_update_args = dict(
            fn=dropdown_prompt_type_list,
            inputs=prompt_type,
            outputs=prompt_type,
        )
        chatbot_update_args = dict(
            fn=chatbot_list,
            inputs=[text_output, model_used],
            outputs=text_output,
        )
        nochat_update_args = dict(
            fn=chatbot_list,
            inputs=[text_output_nochat, model_used],
            outputs=text_output_nochat,
        )
        if not is_public:
            load_model_event = (
                load_model_button.click(
                    **load_model_args,
                    api_name="load_model" if allow_api else None,
                )
        ...
        prompt_update_args2 = dict(
            fn=dropdown_prompt_type_list,
            inputs=prompt_type2,
            outputs=prompt_type2,
        )
        chatbot_update_args2 = dict(
            fn=chatbot_list,
            inputs=[text_output2, model_used2],
            outputs=text_output2,
        )
        if not is_public:
            load_model_event2 = (
        ...
        def dropdown_model_lora_server_list(
            model_list0,
            model_x,
            lora_list0,
            lora_x,
            ...
            ret1 = [
                gr.Dropdown.update(value=x1, choices=model_new_options),
            ...
            ret2 = [
                gr.Dropdown.update(value=x1, choices=lora_new_options),
            ...
            ret3 = [
                gr.Dropdown.update(value=x1, choices=server_new_options),
        ...
        add_model_lora_server_event = add_model_lora_server_button.click(
            fn=dropdown_model_lora_server_list,
        ...
        go_btn.click(
            ...
            None,
            go_btn,
            api_name="go" if allow_api else None,
            queue=False,
        ).then(
            lambda: gr.update(visible=True), None, normal_block, queue=False
        ).then(
            **load_model_args, queue=False
        ).then(
            **prompt_update_args, queue=False
        )
        ...
        compare_checkbox.select(
            compare_textbox_fun,
            compare_checkbox,
            text_output2,
            api_name="compare_checkbox" if allow_api else None,
        ).then(compare_column_fun, compare_checkbox, col_model2).then(
            compare_prompt_fun, compare_checkbox, prompt_type2
        ).then(
            compare_textbox_fun, compare_checkbox, score_text2
        ).then(
            slider_fun, compare_checkbox, max_new_tokens2
        ).then(
            slider_fun, compare_checkbox, min_new_tokens2
        )
        # FIXME: add score_res2 in condition, but do better

        # callback for logging flagged input/output
        callback.setup(
            inputs_list + [text_output, text_output2] + text_outputs,
            "flagged_data_points",
        )
        flag_btn.click(
        ...
        flag_btn_nochat.click(
            lambda *args: callback.flag(args),
            inputs_list + [text_output_nochat],
            None,
            preprocess=False,
        ...
        system_dict_event = system_btn2.click(
        ...
        stop_btn.click(
            ...
            + [eventdb1, eventdb2, eventdb3, eventdb4, eventdb5, eventdb6]
            + [eventdb7, eventdb8, eventdb9],
            queue=False,
            api_name="stop" if allow_api else None,
        ...
        def count_chat_tokens(
            ...
            if tokenizer is not None:
                ...
                return str(
                    tokenizer(context1, return_tensors="pt")[
                        "input_ids"
                    ].shape[1]
                )
            else:
        ...
        demo.load(
            ...
            None,
            None,
            _js=get_dark_js() if kwargs["h2ocolors"] and False else None,
        )  # light best

    demo.queue(
        concurrency_count=kwargs["concurrency_count"],
        api_open=kwargs["api_open"],
    )
    ...
    if is_public and kwargs["base_model"] not in non_hf_types:
    ...
    if (
        ...
        and os.environ.get("TEST_LANGCHAIN_IMPORT")
        and kwargs["base_model"] not in non_hf_types
    ):
        assert (
...
def get_sources(db1, langchain_mode, dbs=None, docs_state0=None):
    ...
    if langchain_mode in ["ChatLLM", "LLM"]:
        ...
        source_files_added = (
            "Not showing wiki_full, takes about 20 seconds and makes 4MB file."
    ...
    sources_file = os.path.join(
        sources_dir, "sources_%s_%s" % (langchain_mode, str(uuid.uuid4()))
    )
...
def update_user_db(
    file, db1, x, y, *args, dbs=None, langchain_mode="UserData", **kwargs
):
    set_userid(db1)

    try:
        return _update_user_db(
            file,
            db1,
            x,
            y,
...
def _update_user_db(
    ...
    if hasattr(file, "name"):
        file = file.name
    if not isinstance(file, (list, tuple, typing.Generator)) and isinstance(
        file, str
    ):
        file = [file]
    ...
    sources = path_to_docs(
        file if not is_url and not is_txt else None,
        verbose=verbose,
        n_jobs=n_jobs,
        chunk=chunk,
...
def get_db(db1, langchain_mode, dbs=None):
    ...
    with filelock.FileLock(lock_file):
        if langchain_mode in ["wiki_full"]:
            ...
            langchain_mode == "MyData" and len(db1) > 0 and db1[0] is not None
        ):
            db = db1[0]
        elif (
            dbs is not None
            and langchain_mode in dbs
            and dbs[langchain_mode] is not None
        ):
...
def get_source_files_given_langchain_mode(
    db1, langchain_mode="UserData", dbs=None
):
...
def get_source_files(db=None, exceptions=None, metadatas=None):
    ...
    small_dict = {
        get_url(x["source"], from_str=True, short_name=True): get_short_name(
            x.get("head")
        )
        for x in metadatas
    }
    ...
    if exceptions:
        ...
        small_dict = {
            get_url(
                x["source"], from_str=True, short_name=True
            ): get_short_name(x.get("exception"))
            for x in exception_metadatas
        }
        # if small_dict is empty dict, that's ok
    ...
    if metadatas and exceptions:
        ...
        """.format(
            source_label, source_files_added, exceptions_html
        )
    elif metadatas:
        source_files_added = """\
        <html>
        ...
        """.format(
            source_label, source_files_added
        )
