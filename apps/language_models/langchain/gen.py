...
def main(
    ...
    allow_upload_to_my_data = bool(
        int(
            os.environ.get(
                "allow_upload_to_my_data", str(int(allow_upload_to_my_data))
            )
        )
    )
    ...
    visible_langchain_modes = ast.literal_eval(
        os.environ.get("visible_langchain_modes", str(visible_langchain_modes))
    )
    if (
    ...
    if cli:
        ...
        return run_cli(
            **get_kwargs(run_cli, exclude_names=["model_state0"], **locals())
        )
    elif not gradio:
        from eval import run_eval

        return run_eval(
            **get_kwargs(run_eval, exclude_names=["model_state0"], **locals())
        )
    elif gradio:
        ...
        for model_dict in reversed(model_list):
            ...
            if base_model1 and not login_mode_if_model0:
                model0, tokenizer0, device = get_model(
                    reward_type=False,
                    **get_kwargs(
                        get_model, exclude_names=["reward_type"], **all_kwargs
                    ),
                )
            else:
        ...
        smodel, stokenizer, sdevice = get_score_model(
            reward_type=True,
            **get_kwargs(
                get_score_model, exclude_names=["reward_type"], **all_kwargs
            ),
        )
...
def get_client_from_inference_server(
    inference_server, base_model=None, raise_connection_exception=False
):
    ...
    if gr_client is None:
        ...
        try:
            ...
            if raise_connection_exception:
                raise
        print(
            "HF Client End: %s %s : %s" % (inference_server, base_model, res)
        )
...
def get_hf_model(
    ...
    config, _ = get_config(
        base_model, return_model=False, raise_exception=True, **config_kwargs
    )
    ...
    if isinstance(tokenizer, str):
        ...
        if "mpt-" in base_model.lower() and gpu_id is not None and gpu_id >= 0:
            ...
            model_kwargs.update(
                dict(device_map={"": gpu_id} if device == "cuda" else "cpu")
            )
...
def get_score_model(
    ...
    if score_model is not None and score_model.strip():
        ...
        llama_type = False
        compile_model = False
        smodel, stokenizer, sdevice = get_model(
            reward_type=True,
...
def evaluate(
    ...
    if inference_server.startswith("openai") or inference_server.startswith(
        "http"
    ):
    ...
    with torch.no_grad():
        ...
        with context_class_cast(device):
            ...
            context_class = (
                NullContext  # if concurrency_count > 1 else filelock.FileLock
            )
            if verbose:
            ...
            with context_class("generate.lock"):
                ...
                if inputs_decoded == prompt:
                    ...
                    if verbose:
                        print(
                            "WARNING: Special characters in prompt", flush=True
                        )
                if stream_output:
                    skip_prompt = False
...
inputs_kwargs_list = [
    ...
    if x not in eval_func_param_names + state_names
]


def get_cutoffs(
    memory_restriction_level, for_context=False, model_max_length=2048
):
...
def score_qa(
    ...
    try:
        score = (
            torch.sigmoid(smodel(**inputs).logits[0]).cpu().detach().numpy()[0]
        )
    except torch.cuda.OutOfMemoryError as e:
        print(
...
def history_to_context(
    ...
    _, _, _, max_prompt_length = get_cutoffs(
        memory_restriction_level1,
        for_context=True,
