...
class Decoder(nn.Module):
    ...
    def forward(self, features, gt_norm_mask=None, mode="test"):
        ...
        x_d1 = self.up1(
            x_d0, x_block3
        )  # x_d1 : [2, 1024, 30, 40]      1/16 res
        x_d2 = self.up2(
            x_d1, x_block2
        )  # x_d2 : [2, 512, 60, 80]       1/8 res
        x_d3 = self.up3(
            x_d2, x_block1
        )  # x_d3: [2, 256, 120, 160]      1/4 res
        x_d4 = self.up4(
            x_d3, x_block0
        )  # x_d4: [2, 128, 240, 320]      1/2 res
        ...
        if mode == "train":
            ...
            feat_map = torch.cat(
                [feat_map, init_pred], dim=1
            )  # (B, 512+4, H, W)
            B, _, H, W = feat_map.shape

            # try all pixels
            out_res4 = self.out_conv_res4(
                feat_map.view(B, 512 + 4, -1)
            )  # (B, 4, N)
        ...
        if mode == "train":
            ...
            out_res2 = self.out_conv_res2(
                feat_map.view(B, 256 + 4, -1)
            )  # (B, 4, N)
        ...
        if mode == "train":
            ...
            out_res1 = self.out_conv_res1(
                feat_map.view(B, 128 + 4, -1)
            )  # (B, 4, N)
